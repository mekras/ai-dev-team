...
- `kc-concepts` ведёт указательный слой согласования межисточниковых понятий.
...
принадлежит анализу и не может маскироваться под прямой `fact`. Слой понятий
хранит связи и разрешение коллизий, но не копирует содержание корпуса или
продукта.
...
3. Для нового корпуса создаётся минимальный переносимый договор.
...
повторной операционной проверке. Одну попытку изменяет только один исполнитель.
